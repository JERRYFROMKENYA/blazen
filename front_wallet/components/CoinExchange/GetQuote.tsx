...
const confirmQuote = async () => {
    ...
    if (res.ok) {
        ...
        const pfi = await pb.collection('pfi').getFirstListItem(`did = "${rfq.metadata.to}"`);


        try {
            const curr_wallet = await pb.collection('wallet').getFirstListItem(`address = "${user.id}" && provider = "${pfi.name}" && currency = "${quote[1].data.payout.currencyCode}"`);
            const new_balance = Number(curr_wallet.balance) + Number(quote[1].data.payout.amount);
            const new_amount_record = await pb.collection('wallet').update(curr_wallet.id, {balance: new_balance});
            ...
            const new_wallet_data = await pb.collection('wallet').create({
                currency:quote[1].data.payout.currencyCode,
                balance:quote[1].data.payout.amount,
                address:user.id,
                provider: pfi.name,
                user:user.id
            })
    ...
    return (
        <>
            {!showResponse ? (
                ...
                <>
                    <Text variant={"bodyMedium"}>NexX will attempt to perform
                        a currency exchange using your current wallet on the tbDEX network </Text>
                    {/*{ console.log(quote)}*/}
                    <Surface elevation={3} style={{ flexDirection:"column",width:"100%",
                        ...
                        <Text variant={"bodyMedium"}>You Pay: {quote[1].data.payin.currencyCode}{" "}
                        ...
                        <Text variant={"bodyMedium"}>You Get: {quote[1].data.payout.currencyCode}{" "}
